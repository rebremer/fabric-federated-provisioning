...
Single Python script that demos a **federated** Fabric provisioning flow with three personas — named after the organizational role that owns each slice, not the identity type — and **three top-level steps** (each composed of named sub-steps). The **Fabric admin** (a human, or an SPN holding the Entra **Fabric Administrator** directory role) runs a one-time bootstrap (tenant allow-list + Azure RG create + gateway Admin grants); the **Platform team** (running as the Platform SPN) then ARM-creates the Fabric capacity, provisions the workspace, and federates gateway access to team security groups; the **Workspace team** (running as the Team SPN) owns its workspace end-to-end.
...
│ Fabric admin                 │  │ Platform team                │  │ Workspace team               │
│ (one-time bootstrap)         │  │ (workspace factory)          │  │ (workspace contents)         │
├──────────────────────────────┤  ├──────────────────────────────┤  ├──────────────────────────────┤
│ runs as: az login            │  │ runs as: Platform SPN login  │  │ runs as: Team SPN login      │
│   human Fabric admin -OR-    │  │   (member of                 │  │   (member of                 │
│   SPN with Entra "Fabric     │  │    platform_workspace_sg +   │  │    team_workspace_sg,        │
│   Administrator" role        │  │    platform_gateway_sg,      │  │    granted by step 2)        │
│                              │  │    granted by step 1)        │  │                              │
├──────────────────────────────┤  ├──────────────────────────────┤  ├──────────────────────────────┤
│ 1.1  tenant allow-list       │  │ 2.1  ARM: Fabric capacity F2 │  │ 3.1a OPDG SQL connection     │
│      for platform_ws_sg      │  │      + self-assign Admin     │  │ 3.1b ADLS Gen2 connection    │
│                              │  │                              │  │      (ShareableCloud)        │
│ 1.2  create capacity RG      │  │ 2.2a create workspace        │  │                              │
│      + Contributor to        │  │      bound to capacity       │  │ 3.2  create / update Fabric  │
│      platform_ws_sg          │  │ 2.2b Contributor on ws       │  │      copy pipeline           │
│                              │  │      to team_sg              │  │      (SQL -> Parquet)        │
│ 1.3a Admin on OPDG           │  │ 2.2c Contributor to          │  │                              │
│      to platform_gw_sg       │  │      Team SPN (direct)       │  │ 3.3  trigger pipeline run    │
│ 1.3b Admin on VDG            │  │                              │  │      and poll until done     │
│      to platform_gw_sg       │  │ 2.3a ConnectionCreator       │  │                              │
│                              │  │      on OPDG to team_sg      │  │                              │
│                              │  │ 2.3b ConnectionCreator       │  │                              │
│                              │  │      on VDG to team_sg       │  │                              │
├──────────────────────────────┤  ├──────────────────────────────┤  ├──────────────────────────────┤
│ writes to                    │  │ writes to                    │  │ writes to                    │
├──────────────────────────────┤  ├──────────────────────────────┤  ├──────────────────────────────┤
│ Fabric tenant settings       │  │ capacity RG (Contributor)    │  │ team workspace items:        │
│   allow-list pf_ws_sg        │  │ Fabric capacity F2 (Admin)   │  │   OPDG SQL connection        │
│                              │  │ team workspace (creates)     │  │   ADLS Gen2 connection       │
│ capacity RG (creates;        │  │ workspace Contributor grants │  │   copy pipeline              │
│   Contributor -> pf_ws_sg)   │  │   -> team_sg + Team SPN      │  │   pipeline run               │
│                              │  │                              │  │                              │
│ OPDG Admin -> pf_gw_sg       │  │ OPDG ConnectionCreator       │  │ no access to:                │
│ VDG  Admin -> pf_gw_sg       │  │ VDG  ConnectionCreator       │  │   capacity RG, capacity,     │
│                              │  │   both -> team_sg            │  │   gateway admin settings     │
│                              │  │                              │  │                              │
│                              │  │ no access to:                │  │                              │
│                              │  │   Fabric tenant settings,    │  │                              │
│                              │  │   OPDG/VDG admin settings    │  │                              │
└──────────────────────────────┘  └──────────────────────────────┘  └──────────────────────────────┘
       step 1 grants SG perms ─────────►  step 2 consumes them, then  ─────────►  step 3 consumes
       (pf_ws_sg, pf_gw_sg)               grants team_sg perms                    team_sg perms
```
...
| 3.2 | Workspace team — *pipeline* | Creates / updates the copy pipeline. |
| 3.3 | Workspace team — *run pipeline* | Triggers the pipeline run and polls to completion. |

After step 1, onboarding a new team workspace = the Platform team running `python scripts/provision_fabric.py 2 config/<new>.yaml` (as the Platform SPN), then the Workspace team running `... 3 ...` (as the Team SPN).
...
- The identity running step 1 (human Fabric admin *or* a bootstrap SPN — both work; the script and the underlying Fabric / ARM APIs don't care which) needs:
  - **Fabric service administrator** role (Entra **Fabric Administrator** directory role for an SPN — for 1.1 tenant-settings + 1.3 gateway role assignments) and `Tenant.ReadWrite.All`
...
# Step 2 — Platform team (runs as the Platform SPN)
az logout
...
# Step 3 — Workspace team (runs as the Team SPN, member of team_workspace_contributor_security_group)
az logout
